...
pub fn op_8(emulator: &mut Emulator, opcode: u16) -> Result<()> {
    match opcode & 0x000F {
        ...
        0x0004 => {
            ...
            let sum = emulator.v_regs[vx] as u16
        ...
        0x0005 => {
            ...
            let diff = emulator.v_regs[vx] as i16
                - emulator.v_regs[((opcode & 0x00F0) >> 4) as usize] as i16;
        ...
        0x0007 => {
            ...
            let diff = emulator.v_regs[((opcode & 0x00F0) >> 4) as usize] as i16
                - emulator.v_regs[vx] as i16;
...
fn shift_vx_quirk(vx: usize, opcode: u16, emulator: &mut Emulator) -> Result<u8> {
    if !emulator.config.quirks.shift_vx_directly {
        Ok(emulator.v_regs[((opcode & 0x00F0) >> 4) as usize])
    } else {
        Ok(emulator.v_regs[vx])
    }
}
